...
import { updateHousehold } from "../../api/households/householdService";
...
type Props = NativeStackScreenProps<AuthStackParamList, "UpdateHousehold">;

export default function UpdateHouseholdScreen({ navigation, route }: Props) {
  ...
  const onSubmit = async () => {
    ...
    await updateHousehold(
      ...
      (err: ApiError) => {
        ...
        setGlobalErrors(err.globalErrors ?? [t("updateHousehold.defaultError")]);
      }
    );
      ...
      <View style={styles.screen}>
        ...
        >
          ...
          <Text style={styles.optionalLabel}>
            ...
            <Text style={styles.optionalText}>({t("createHousehold.optional")})</Text>
          </Text>
          ...
          <View style={styles.footer}>
            <PrimaryButton
              text={t("updateHousehold.submit")}
              onPress={onSubmit}
              disabled={submitting}
              rightIcon="content-save-outline"
            />
            <Pressable onPress={() => navigation.goBack()} hitSlop={8} style={styles.cancelBtn}>
